...
- `strcut(text, maxlen = 50)` — укорачивает длинную строку, сохраняя начало и конец через разделитель `'… ⋯ …'`. Левая часть забирает ~65% полезной длины (удобно для сохранения протокола/домена слева и имени/расширения файла или GET-параметров справа). Для не-строк или если длина меньше `maxlen` — возвращает оригинал.
